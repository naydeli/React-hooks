...
export default function App() {
  ...
  return (
    <>
      <header className="flex items-center justify-between p-2 text-lg text-center text-white uppercase border-b-8 shadow-lg select-none bg-green-400 border-green-700 shadow-green-200">
        <span
          onClick={() => setMostrar(1)}
          className="hover:cursor-pointer hover:text-green-700 active:text-green-400">
          <MdMenu size={40} />
        </span>
        ...
        <div
          ...
          } inset-0 z-20 bg-green-200 bg-opacity-50 backdrop-blur-sm transition-all duration-500 ease-in-out`}></div>

        <aside
          className={`h-screen w-11/12 lg:w-3/12 p-4 fixed
        top-0 left-0 bottom-0 bg-white
        shadow-lg shadow-gray-500 text-green-500 z-50 transition duration-500 ease-in-out
          ...
          <h2 className="flex items-center justify-between text-2xl font-bold">
            <span>
              <MdArrowBack
                size={40}
                onClick={() => setMostrar(0)}
                className="hover:cursor-pointer hover:text-green-600 active:text-green-100"
              />
            </span>
